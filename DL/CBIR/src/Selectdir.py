...
class Selectdir(object):
    ...
    #  创建界面
    def Createpage(self):
        inp1 = tk.Entry(self.root, font=("Ubuntu", 18))
        inp1.pack()
        inp1.place(relx=0.2, rely=0.3, relwidth=0.6, relheight=0.1)

        #  添加一些小图标
        imgs = Image.open(r'./src/image/sousuo.png')
        ...
        imgt = Image.open(r'./src/image/tupian.png')
        ...
        imgc = Image.open(r'./src/image/chengyuan.png')
        ...
        #  本地上传图标
        databutton = tk.Button(self.root, text="upload",font=("Ubuntu", 20), command=lambda: Choosedatadir(inp1))
        databutton.place(relx=0.8, rely=0.3, relwidth=0.1, relheight=0.1)
        #  开始检索图标
        enterbutton = tk.Button(self.root, text="start",font=("Ubuntu", 20), command=lambda: Enter())
        ...
        def Enter():
            ...
            result.title('查询结果')
            photo = tk.PhotoImage(file=r"./src/image/a.gif")  # 背景图片

            background = tk.Label(result, image=photo, compound=tk.CENTER)
            background.place(relx=0, rely=0, relwidth=1, relheight=1)

            backbutton = tk.Button(result, text="back", font=("Ubuntu", 25), command=lambda: Back(result))
            backbutton.place(relx=0.8, rely=0.1, relwidth=0.08, relheight=0.08)

            word1 = tk.Label(result, text='The pictures to be retrieved are as follows', font=("Ubuntu", 15), image=photo, compound=tk.CENTER)
            word1.place(relx=0.1, rely=0, relwidth=0.4, relheight=0.07)

            word2 = tk.Label(result, text='The search results are as follows', font=("Ubuntu", 15), image=photo, compound=tk.CENTER)
            word2.place(relx=0.15, rely=0.4, relwidth=0.3, relheight=0.07)
        ...
        def Back(result):
            ...
            lb1 = tk.Label(root, text='Please enter the image you want to retrieve', font=("Ubuntu", 10), image=photo, compound=tk.CENTER)
