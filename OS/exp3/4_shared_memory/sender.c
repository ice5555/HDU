...
void my_init(){
    ...
    //key关键字，共享内存标识符，size申请1k字节大小，flag为权限标识 
    //用来启动对该共享内存的访问，并把共享内存连接到当前进程的地址空间
    shmp = shmat(shmid, NULL, 0);
    //shmat函数用来启动对该共享内存的访问，并把共享内存连接到当前进程的地址空间。
    //shmid共享内存的ID shmaddr共享内存的起始地址 shmflag本进程对该内存的操作模式
}
...
int main(){
    ...
    while(1){
        ...
        if(strcmp(send,"exit")==0)  flag=1;

        memset((char *)shmp, '\0', 1024);//清空共享内存空间
        strcpy((char *)shmp, send);//将send内容拷贝至共享内存空间 
