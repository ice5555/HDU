...
void my_format(){
    ...
    //3. 设置根目录文件的两个特殊目录项.和..(根目录文件占一个盘块,两个目录项是在写在这个盘块里面的)（1个盘块、第6块）
...
void my_mkdir(char *dirname){
    ...
    //在对应的盘块里添加两个特殊的目录.和..
    //同理这里也不能直接写进fcbPtr里,只能用fcbTmp写进内存里,然后用do_write转写进文件
    fcbtmp->metadata = 0;
...
int  my_close(int fd){
    ...
    if(fd > MAXOPENFILE || fd < 0){
        ...
        //写回到磁盘的时候,只要让游标filePtr指向这个文件的对应fcb的位置,然后写入fcb就好了
        //这样只要写一个fcb大小的数据就行了
...
int  do_write(int fd, char *text, int len, char wstyle){
    //首先这是文件系统,所以不能通过blockPtr = myvhard + BLOCKSIZE * blockNum 定位到盘块,然后就直接在盘块上写数据了!!
    //用一个buf, 把盘块内容读取进来, 然后在buf里面修改, 然后再把buf内容写回到盘块里去
    //这里会有两个循环. 在覆盖写的情况下, 会先找到openfilelist[].filePtr, 也就是游标, 指向文件现在写到哪了
    //1. 如果filePtr>BLOCKSIZE, 这就意味着游标指向的不是第一个盘块, 所以你要通过FAT找到filePtr指向的盘块,如果中途发现有些盘块是END
    //   那么你需要占用这些盘块,一直占用到游标filePtr指向的盘块(比如filePtr = 5*BLCOKSIZE + x, 那么要一直往下找5个盘块)
    //2. 如果len>BLCOKSIZE-off, 那么改写的内容还没写完, 你要申请盘块, 继续往下写, 一直写到len个字符
    //PS: 在my_write中有3中写法,
    // 0:截断写, 也就是把文件内容全部删除重新写.
    // 1:覆盖写:也就是从filePtr指向的内容继续往下写
    // 2:追加写,从文件的最后继续写
